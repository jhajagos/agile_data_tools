    ...
    for original_label in raw_header:  # Rewrite column names in a more SQL friendly way
        ...
        for split_char in special_characters_map.keys():
            ...
            if len(split_label) > 1:
                label = special_characters_map[split_char].join(
                    split_label)  # split_label.join(special_characters_map[split_char])
...
def generate_schema_from_csv_file(file_name, connection_url, table_name="temp_table", delimiter=",", no_header=False,
    ...
    with open(file_name, newline="", mode="r", errors="replace") as f:
        ...
        for j in range(len(header)):
            ...
            if data_type[
                column_name] == Integer:  # If the integer is too large store as string using 2**32 has 10 digits as cut off
        ...
        if schema_only:
            ...
            import_csv_file_using_inserts(file_name, connection_url, table_name, header, data_type, positions,
...
re_float_complex = re.compile(
    r"-?([0-9]*\.[0-9]+[eE](\+|\-)?[0-9]+|[[1-9][0-9]*[eE](\+|\-)?[0-9]+|[0-9]*\.[0-9]*|\.[0-9]+[eE](\+|\-)?[0-9]+|\.[0-9]+|[1-9][0-9]*)$")
...
def ensure_options_dict_missing_fields(options_dict):
    option_names = ["file_name", "connection_string", "table_name", "delimiter", "no_headers", "header",
                    "out_file_name",
